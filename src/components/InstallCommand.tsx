...
export default function InstallCommand() {
  ...
  return (
    <div className="inline-flex items-center bg-brand-blue font-mono text-sm text-white">
      ...
      >
        <Select.Trigger className="group flex items-center gap-1.5 bg-black/3 px-4 py-3 text-brand-blue cursor-pointer transition-colors hover:bg-brand-blue-50">
          <Select.Value />
        ...
        <Select.Portal>
          <Select.Positioner sideOffset={4} align="start" alignItemWithTrigger={false}>
            <Select.Popup className="min-w-40 border border-black/10 bg-white py-1 font-mono text-sm text-brand-blue shadow-[0_12px_32px_-12px_rgba(0,0,0,0.18)]">
              {OS_ITEMS.map((item) => (
                <Select.Item
                  key={item.value}
                  value={item.value}
                  className="cursor-pointer px-3 py-2 data-[highlighted]:bg-black/3"
                >
      ...
      <code className="px-4 normal-case">{copied ? "Copied!" : COMMANDS[os]}</code>

      <button
        type="button"
        onClick={handleCopy}
        aria-label={copied ? "Copied" : "Copy install command"}
        className="flex shrink-0 items-center justify-center border-l border-white/20 px-3 py-3 cursor-pointer transition-colors hover:bg-brand-blue-400 active:scale-[0.985]"
      >
        <ClipboardIcon />
      </button>
    </div>
